...
class customBert(object):
    """
    The main class for customBert.
    """
    ...
    def prepare_model(self, label_list):
        ...
        self.processor = self.processors['sentiment']()
    ...
    def get_data(self, phase):
        ...
        if phase=='train':
            train = pd.read_csv(os.path.join(self.config.data_dir, 'train.csv'),encoding = "utf-8",index_col=False)
    ...
    def train(self, train_examples, model):
        ...
        for _ in trange(int(self.config.num_train_epochs), desc="Epoch"):
            ...
            self.training_losses.append(tr_loss/nb_tr_steps)
            print("Training losses: {}".format(self.training_losses))
            ...
            for input_ids, input_mask, segment_ids, label_ids in tqdm(validation_loader, desc="Validating"):
    ...
    def evaluate(self, model, examples):
        ...
        for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_loader, desc="Testing"):
            input_ids = input_ids.to(self.device)
            input_mask = input_mask.to(self.device)
            segment_ids = segment_ids.to(self.device)
            label_ids = label_ids.to(self.device)
        ...
        evaluation_df = pd.DataFrame({'predictions': predictions, 'labels': labels})

        return evaluation_df
    

def predict_project_special(text, model, write_to_csv=False, path=None):
    """
